...
import { Eye, Edit, Trash2, ChevronsUpDown, Loader2 } from "lucide-react";
...
export default function UsersTable() {
  ...
  const { data, isLoading, isError, error } = useUsers(currentPage, rowsPerPage);
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        <div className="flex justify-between items-center mt-4">
          <div className="text-sm text-muted-foreground">
            ...
            {Math.min(((data?.number ?? 0) + 1) * (data?.size ?? rowsPerPage), data?.totalElements ?? 0)} of{" "}
            ...
            {Array.from({ length: data?.totalPages ?? 0 }, (_, i) => i).map((page) => (
              <button
                ...
                className={`px-3 py-1 text-sm rounded border hover:bg-gray-100 ${
                  page === (data?.number ?? 0) ? "bg-blue-500 text-white hover:bg-blue-600" : ""
                }`}
              >
                {page + 1}
              </button>
